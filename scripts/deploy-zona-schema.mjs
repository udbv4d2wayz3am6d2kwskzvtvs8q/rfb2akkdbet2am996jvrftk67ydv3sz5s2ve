...
const query = await readFile(new URL("../supabase/functions/zona/schema.sql", import.meta.url), "utf8");
...
if (!response.ok) {
  ...
  throw new Error(`Zona schema ${projectRef}: ${response.status} ${detail}`);
}
console.log(`Zona schema ready: ${projectRef}`);
