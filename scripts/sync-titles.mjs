...
//  - a rotating 80-page reconciliation slice. Twelve scheduled runs cover the
//    whole catalogue in about a day without hammering the source for 25–40
//    minutes at a time;
...
// are written (titles_upsert_catalog), so reconciliation costs the database
...
const RECONCILE_PAGES_PER_RUN = 80;
const FILL_BATCH = 25;
...
export async function syncCatalog({ full = false, deadline = Infinity, startPage = 1,
  checkpoint = null, maxPages = Infinity } = {}) {
  const stats = { pages: 0, inserted: 0, updated: 0, total: 0, reachedEnd: false, nextPage: startPage };
  let lastFirst = "";
  let quiet = 0;
  try { for (let page = startPage; net.now() < deadline && stats.pages < maxPages; page += 1) {
    if (page > startPage) await net.sleep(SPACING_MS);
    const query = new URLSearchParams({
      findBy: "filter", all: "false", page: String(page), "per-page": String(PER_PAGE),
      sortBy: "priority", _format: "json",
    });
...
export async function runSync({ forceFull = false, auto = true, budgetMin = 45, fillLimit = 300 } = {}) {
  ...
  let catalog;
  if (forceFull) {
    const started = new Date(state.full_started_at || net.now()).toISOString();
    catalog = await syncCatalog({
      full: true, deadline: catalogDeadline,
      checkpoint: auto ? (page) => net.titles("/sync-state", { next_full_page: page, full_started_at: started }) : null,
    });
    if (auto && catalog.reachedEnd) await net.titles("/sync-state", { completed: true, full_started_at: started });
  } else {
    // New ids are monotonic and the listing starts with the newest releases.
    // This is the actual incremental path: stop after three unchanged pages.
    head = await syncCatalog({ deadline: catalogDeadline });
    catalog = head;
    if (auto && net.now() < catalogDeadline) {
      await net.sleep(SPACING_MS);
      // The source exposes no updated-since cursor, usable ordering by update
      // time, ETag, or conditional 304. A bounded rotating slice is therefore
      // the cheapest honest way to catch edits and rare old insertions.
      const startPage = Math.max(HEAD_MIN_PAGES + 1, Number(state.next_full_page) || 1);
      const started = new Date(state.full_started_at || net.now()).toISOString();
      catalog = await syncCatalog({
        full: true, startPage, deadline: catalogDeadline, maxPages: RECONCILE_PAGES_PER_RUN,
        checkpoint: (page) => net.titles("/sync-state", { next_full_page: page, full_started_at: started }),
      });
      // Freshness starts when the rotating pass starts, not at its last slice.
      if (catalog.reachedEnd) await net.titles("/sync-state", { completed: true, full_started_at: started });
    }
  }
  if (head) console.log(`head: ${head.pages} pages, ${head.inserted} new, ${head.updated} changed`);
  console.log(`catalogue: ${catalog.pages} pages, ${catalog.inserted} new, ${catalog.updated} changed` +
    ` (source lists ${catalog.total}${forceFull ? `, full read ${catalog.reachedEnd ? "complete" : "cut short"}` :
      auto ? `, reconciliation ${catalog.reachedEnd ? "cycle complete" : "slice complete"}` : ""})`);
  ...
  if (auto && !forceFull && !catalog.reachedEnd) console.log(`reconciliation checkpoint saved near page ${catalog.nextPage}; next run continues`);
  if (fill.stoppedBy && fill.stoppedBy !== "budget") throw new Error(`fill stopped: ${fill.stoppedBy}`);
  return { head, catalog, fill };
}
